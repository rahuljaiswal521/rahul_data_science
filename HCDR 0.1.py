...
def bureau_and_balance(num_rows = None, nan_as_category = True):
    ...
    bureau_agg = pd.merge(bureau_agg,active_agg, how='left', left_on='BURO_SK_ID_CURR_', right_on='ACTIVE_SK_ID_CURR_')
    ...
    bureau_agg = pd.merge(bureau_agg,closed_agg, how='left', left_on='BURO_SK_ID_CURR_', right_on='CLOSED_SK_ID_CURR_')
...
def previous_applications(num_rows = None, nan_as_category = True):
    ...
    prev_agg = pd.merge(prev_agg,approved_agg, how='left', left_on='PREV_SK_ID_CURR_',right_on='APPROVED_SK_ID_CURR_')
    # Previous Applications: Refused Applications - only numerical features
    refused = prev[prev['NAME_CONTRACT_STATUS_Refused'] == 1]
    refused_agg = refused.groupby('SK_ID_CURR', as_index=False).agg(num_aggregations)
    refused_agg.columns = pd.Index(['REFUSED_' + e[0] + "_" + e[1].upper() for e in refused_agg.columns.tolist()])
    prev_agg = pd.merge(prev_agg,refused_agg, how='left', left_on='PREV_SK_ID_CURR_',right_on='REFUSED_SK_ID_CURR_')
...
def credit_card_balance(num_rows = None, nan_as_category = True):
    ...
    cc_agg = cc.groupby('SK_ID_CURR', as_index=False).agg(['min', 'max', 'mean', 'sum', 'var'])
...
xgb1.fit(train_df[feats], train_df['TARGET'])

train_predictions = xgb1.predict(train_df[feats])

roc_auc_score(train_df['TARGET'], train_predictions)
test_df['TARGET'] = xgb1.predict(test_df[feats])

test_df[['SK_ID_CURR', 'TARGET']].to_csv(submission_file_name, index= False)

if [test_df['TARGET'] <0]:
    test_df.loc['TARGET']=test_df[['TARGET']]*-1
    
test_df['TARGET'].loc[test_df['TARGET'] <0]

for i, x in test_df.iterrows():
    if test_df.loc[i,'TARGET']<0:
        print(i)
        test_df.loc[i,'TARGET']=test_df.loc[i,'TARGET']*-1
